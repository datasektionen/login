...
class CASRequestHandler(BaseHTTPRequestHandler):
    ...
    def login(self):
        ...
        if username is None or password is None:
            resp = b'<!DOCTYPE html><html><head></head><body><form action="/login" method="GET"><input name="username" type="text" placeholder="Kanske juland?"><input name="password" type="text" placeholder="Kanske rudolf?"><input name="service" type="hidden" value="%s"><input type="submit"></form></body></html>' % service.encode()
            ...
            return
        if password != self.server.secret:
            self.send_error(403, 'Forbidden')
            return

        ticket = self.server.generate_ticket(service, username)
